...
function Coupon() {
    ...
    const fetchCoupons = useCallback(async () => {
        try {
            ...
            Toast().fire({
                icon: 'error', title: 'Failed to fetch coupons'
            });
        }
    }, [api]);
    ...
    const handleShow = (coupon, deleteMode = false) => {
    ...
    const handleCouponFormChange = (e) => {
        ...
        setCouponForm(prev => ({
            ...prev, [name]: name === 'discount' ? Number(value) : value
        }));
    };
    ...
    const handleCouponSubmit = async (e) => {
        ...
        try {
            ...
            await Toast().fire({
                icon: 'success', title: 'Coupon Created Successfully'
            });
        } catch (error) {
            await Toast().fire({
                icon: 'error', title: 'Failed to create coupon'
            });
        }
    };
    ...
    const handleDeleteCoupon = async (couponId) => {
        try {
            ...
            handleClose();
            await Toast().fire({
                icon: 'warning', title: 'Coupon Deleted Successfully'
            });
        } catch (error) {
            await Toast().fire({
                icon: 'error', title: 'Failed to delete coupon'
            });
        }
    };
    ...
    const handleCouponUpdate = async (e) => {
        ...
        try {
            ...
            handleClose();
            Toast().fire({
                icon: 'success', title: 'Coupon updated Successfully'
            });
        } catch (error) {
            Toast().fire({
                icon: 'error', title: 'Failed to update coupon'
            });
        }
    };
    ...
    return (
                        ...
                        <div className="card mb-4">
                            ...
                            <div className="card-body">
                                <ul className="list-group list-group-flush">
                                  ...
                                  Discount:{" "}
                                  </span><span className="fw-light">{coupon.discount}% Discount</span>
                                                        </p>
                                                        <p className="mt-1">
                                                          <span className="me-2 fw-bold">
                                  Date Created:{" "}
                                                        ...
                                                        <p>
                                                            ...
                                                            <button
                                                                onClick={() => handleShow(coupon, true)}
        ...
        <Modal show={show} onHide={handleClose}>
            <Modal.Header closeButton>
                <Modal.Title>
                    {isDeleteMode ? 'Confirm Deletion' : 'Update Coupon'} - <span
                    className="fw-bold">{couponForm.code}</span>
                </Modal.Title>
            </Modal.Header>
            <form onSubmit={handleCouponUpdate}>
                <Modal.Body>
                    {isDeleteMode ? (<p>Are you sure you want to delete this coupon?</p>) : (<>
                        <div className="mb-3">
                ...
                <Modal.Footer>
                    {isDeleteMode ? (<>
                        ...
                        <Button variant="danger" onClick={() => handleDeleteCoupon(selectedCouponId)}>
                            Delete
                        </Button>
                    </>) : (<>
